...
// vid of datafiles + variables + datafile_sites
 ...
 if ($flatten_files) {
  ...
  // $acr from config --LTER centric, but can easily be expanded
...
//      views_bonus_eml_print_value('abstract', $dataset_abstract);
      ...
      //  following tag not needed here : commented out for now - u can delete !!! ISG
      // views_bonus_eml_print_value('field_dataset_id', $dataset_id);

      // related links are missplaced here, they should go INSIDE additional information:
      // commented out, but we should move them !!!ISG
      // views_bonus_eml_print_value('related_links',    $dataset_related_links);

      // project (from CCT research_project). !!! ISG

      // access tag group - from config file, or from site variable, or... here is my take !!! ISG new next 10 lines or so.
      //  !!! ISG -- feel free to clean the code below...
?>
...
<?php
      ...
      if ($dataset_node[dataset_datafiles] && $dataset_node[dataset_datafiles][0][datafile]->nid) {
        foreach ($dataset_node[dataset_datafiles] as $file_var_array) {
          ...
          if ($file_data_file) {
          ...
          }else{  //!!! ISG to rescue EML validity -- IM not sure how.. we can always make the field mandatory.
            ...
            if ($file_data_file) {
              foreach ($file_data_file as $file_data) {
//                ??? the same as for entityName  !!!ISG YES
               ...
               views_bonus_eml_print_open_tag('textFormat');
                 ...
                 views_bonus_eml_print_open_tag('simpleDelimited');
                   if ( $file_delimiter){
                     ...
                     $file_delimiter=',';
                     views_bonus_eml_print_tag_line('fieldDelimiter',     $file_delimiter);// rescue EML validation.
                   }
